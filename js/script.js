import { doc, getDoc, setDoc, onSnapshot } from 'https://www.gstatic.com/firebasejs/12.6.0/firebase-firestore.js';
...
taskForm.addEventListener('submit', (e) => {
    ...
    if (!name || !url) return;
    let domain = '';
    try { domain = new URL(url).hostname; } catch (e) { domain = 'google.com'; }
    const icon = iconUrl || `https://www.google.com/s2/favicons?domain=${domain}&sz=32`;
    const newTask = { id: Date.now().toString(), name, url, icon, checked: false };
    tasks.push(newTask);
    saveTasks();
    hideModal();
});
